...
const TABLE = process.env.TABLE_NAME || 'forge-pt-sessions';
...
export async function handler(event) {
  ...
  try {
    ...
    if (method === 'GET' && pathParams.id) {
      const item = await getItem(TABLE, { trainerId, sessionId: pathParams.id });
    ...
    if (method === 'PUT' && pathParams.id) {
      await updateItem(TABLE, { trainerId, sessionId: pathParams.id }, {
        ...body,
        updatedAt: new Date().toISOString(),
      });
    ...
    if (method === 'DELETE' && pathParams.id) {
      await deleteItem(TABLE, { trainerId, sessionId: pathParams.id });
      return ok({ success: true });
    }
