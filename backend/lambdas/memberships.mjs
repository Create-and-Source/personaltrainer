...
export async function handler(event) {
  ...
  try {
    ...
    if (method === 'GET' && id) {
      const item = await getItem(TABLE, { trainerId, membershipId: id });
    ...
    if (method === 'PUT' && id) {
      await updateItem(TABLE, { trainerId, membershipId: id }, {
        ...body,
        updatedAt: new Date().toISOString(),
      });
    ...
    if (method === 'DELETE' && id) {
      await deleteItem(TABLE, { trainerId, membershipId: id });
      return ok({ success: true });
    }
