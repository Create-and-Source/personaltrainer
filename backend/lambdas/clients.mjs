...
export async function handler(event) {
  ...
  try {
    ...
    if (method === 'GET' && pathParams.id) {
      const item = await getItem(TABLE, { trainerId, clientId: pathParams.id });
    ...
    if (method === 'PUT' && pathParams.id) {
      await updateItem(TABLE, { trainerId, clientId: pathParams.id }, {
        ...body,
        updatedAt: new Date().toISOString(),
      });
    ...
    if (method === 'DELETE' && pathParams.id) {
      await deleteItem(TABLE, { trainerId, clientId: pathParams.id });
      return ok({ success: true });
    }
