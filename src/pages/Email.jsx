              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10 }}>
                {Object.entries(TEMPLATES).map(([key, tpl]) => (
                  <button key={key} onClick={() => selectTemplate(key)} style={{
                    ...
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: template === key ? s.accentLight : 'rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', font: `600 14px ${s.FONT}`, color: template === key ? s.accent : s.text3, marginBottom: 8 }}>{tpl.name[0]}</div>
