...
function generateNotifications() {
  ...
  retentionAlerts.filter(a => a.status === 'pending').forEach(alert => {
    notifications.push({
      ...
      type: 'retention',
      icon: 'retention',
      text: `${alert.patientName} hasn't attended in ${alert.daysSince} days`,
  ...
  if (todayAppts.length > 0) {
    notifications.push({
      ...
      type: 'appointment',
      icon: 'appointment',
      text: `${todayAppts.length} class${todayAppts.length !== 1 ? 'es' : ''} today`,
      link: '/admin/schedule',
      time: now,
      color: '#16A34A',
    });
  ...
  if (pendingWaivers.length > 0) {
    ...
    pendingWaivers.slice(0, 2).forEach(w => {
      notifications.push({
        ...
        type: 'waiver',
        icon: 'waiver',
        text: `${w.patientName || 'A member'} has unsigned waivers`,
        link: '/admin/waivers',
  ...
  if (waiting.length > 0) {
    notifications.push({
      ...
      type: 'waitlist',
      icon: 'waitlist',
      text: `${waiting.length} member${waiting.length !== 1 ? 's' : ''} on waitlist`,
      link: '/admin/waitlist',
      time: now,
      color: '#7C3AED',
    });
